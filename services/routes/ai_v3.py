"""沉浸式 AI 会话 v3 接口（设计稿 §11.2 / §11.4）

新增独立路径（v2 零改动、小程序零改动，§11）：
- `POST /ai/session/v3` — 提交会话生成任务（mode=start 开场 / mode=turn 续轮），
  请求字段与业务码**对齐 v2**；新增总开关 `SESSION_V3_ENABLED`（默认 0），
  关闭时提交返回 200 + success=false + code=SESSION_V3_DISABLED。
- `GET /ai/session/v3/task/{task_id}` — 查询生成结果（同 v2 状态机/TTL/卡死自愈）。

差异仅在**路径与集合**（§11.4）：`/ai/session/v3` + `ai_session_v3*`；
小程序切换时只需替换端点常量，响应解析零改动。
...
入参核心字段可选化 + 手动校验 → 业务失败 HTTP 200 + success=false + code：
  ...
  TYPE_NOT_SUPPORTED（preferred_type=retell/task，MVP 边界）/ SESSION_V3_DISABLED（总开关关闭）；
技术异常 → HTTP 500。
"""
...
from config import SESSION_V3_ENABLED
...
from services.learning.session_state_v3 import (
    build_session_id,
    create_session,
    get_session,
    set_pending,
)
from services.learning.session_task_v3 import (
    COLLECTION,
    build_task_id,
    create_session_task,
    get_task,
...
logger = logging.getLogger("scholar-admin.ai_v3")

router = APIRouter(tags=["ai"])

# 后台任务强引用集合：防止 create_task 的协程被 GC 回收导致任务中途取消（同 v2）
_background_tasks: set[asyncio.Task] = set()

# MVP 形态白名单（§11.4：auto/dialogue/fill；retell/task → TYPE_NOT_SUPPORTED）
...
class AiSessionV3Request(BaseModel):
    """会话 v3 提交请求（字段与口径同 v2，§11.4）。"""
...
# ---------------------------------------------------------------------------
# 手动校验 helpers（业务失败 200 + success=false，对齐 v2 / §11.4）
# ---------------------------------------------------------------------------
...
def _validate_scholar_id(body: AiSessionV3Request) -> AiSessionResponse | None:
...
def _validate_preferred_type(body: AiSessionV3Request) -> AiSessionResponse | None:
...
def _validate_start_payload(body: AiSessionV3Request) -> AiSessionResponse | None:
    """start 模式必填结构校验：scenario / roles / groups（含叶子长度口径，同 v2）。"""
...
def _build_start_context(scenario: dict, roles: dict, groups: list) -> dict:
    """start 上下文快照：mode=start + 场景/角色/素材，history 为空（同 v2）。"""
    return {
        "mode": "start",
        "scenario": scenario,
        "roles": roles,
...
# ---------------------------------------------------------------------------
# 提交：POST /ai/session/v3
# ---------------------------------------------------------------------------


@router.post("/ai/session/v3", response_model=AiSessionResponse)
async def submit_ai_session_v3(
    body: AiSessionV3Request,
    db: CloudBaseNoSQLClient = Depends(get_db),
) -> AiSessionResponse:
    """提交会话 v3 生成任务 — 毫秒级返回 task_id/session_id，生成在后台执行。

    - 总开关关闭（SESSION_V3_ENABLED=0）→ SESSION_V3_DISABLED（零侵入可一键回退）；
    - mode=start：校验三要素 → 建会话态（ai_session_v3）+ 任务（占在途位）→ 调度；
    - mode=turn：装载会话态（归属校验 + 在途位 gate）→ 建任务（context 自包含快照）→ 调度；
      user_input 为空 → INVALID_INPUT。
    """
    # 0. 总开关（默认 0 关闭；不影响已创建任务的查询）
    if not SESSION_V3_ENABLED:
        return _fail("SESSION_V3_DISABLED", "会话 v3 未启用（SESSION_V3_ENABLED=0）")
    ...
    try:
        ...
        logger.error(f"[ai_v3] session v3 任务创建异常: {e}", exc_info=True)
    ...
    logger.info(
        f"[ai_v3] session v3 任务已提交 → task_id={task_id}, mode={mode}, "
...
# ---------------------------------------------------------------------------
# 查询：GET /ai/session/v3/task/{task_id}
# ---------------------------------------------------------------------------


@router.get("/ai/session/v3/task/{task_id}", response_model=AiSessionResponse)
async def get_ai_session_v3_task(
    task_id: str,
    db: CloudBaseNoSQLClient = Depends(get_db),
) -> AiSessionResponse:
    """查询异步会话 v3 生成结果 — pending/processing/success/failed（同 v2）。"""
    try:
        ...
        logger.error(f"[ai_v3] session v3 任务查询异常: task_id={task_id}: {e}", exc_info=True)
