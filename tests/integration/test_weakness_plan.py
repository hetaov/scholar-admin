...
def _seed_content(fake_db):
    """双教材内容层级：tb_1(c1: l1(s1,s2), l2(s3,s4)) + tb_2(c2: l3(s5,s6))。"""
    seed_content(fake_db)
    seed_content(fake_db, textbook_id="tb_2", chapter_id="c2", lesson_ids=("l3",), sentence_ids=("s5", "s6"))
...
class TestWeaknessPlan:
    ...
    def test_low_score_and_not_started_filter(self, make_client, fake_db):
    ...
    def test_weakest_skill_pick_is_used_for_score(self, make_client, fake_db):
    ...
    def test_scope_lesson(self, make_client, fake_db):
    ...
    def test_empty_queue(self, make_client, fake_db):
    ...
    def test_lesson_without_textbook_400(self, make_client, fake_db):
        client = make_client(tracking_router)
        resp = client.post(
            "/tracking/weakness-plan",
