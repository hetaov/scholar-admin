...
from tests.fakes.fake_providers import FakeTtsProvider
...
def _client(make_client, provider=None):
    """构建 TestClient：make_client 注入 FakeDB，TTS Provider 走 dependency_overrides。"""
    if provider is None:
        provider = FakeTtsProvider()
    return make_client(tts_router, overrides={get_tts_provider: lambda: provider})


class TestTtsSuccess:
    def test_normal_synthesis_and_persist(self, make_client, fake_db):
    ...
    def test_sentence_id_persisted(self, make_client, fake_db):
    ...
    def test_provider_receives_normalized_text(self, make_client, fake_db):
        """Provider 收到的 text 应为 normalize 后（trim + 合并连续空白）。"""
        provider = FakeTtsProvider()
        client = _client(make_client, provider=provider)
...
class TestTtsCache:
    def test_cache_hit_second_request(self, make_client, fake_db):
    ...
    def test_cache_hit_on_normalized_variant(self, make_client, fake_db):
    ...
    def test_ref_count_update_failure_does_not_block_cache(self, make_client, fake_db, monkeypatch):
...
class TestTtsValidation:
    def test_empty_text(self, make_client, fake_db):
    ...
    def test_blank_text(self, make_client, fake_db):
    ...
    def test_text_too_long(self, make_client, fake_db):
    ...
    def test_missing_text_422(self, make_client, fake_db):
...
class TestTtsFallback:
    def test_provider_unavailable(self, make_client, fake_db):
        ...
        client = _client(make_client, provider=FakeTtsProvider.unavailable())
        resp = client.post("/audio/tts", json={"text": DEFAULT_TEXT})
        body = resp.json()
        assert body["success"] is False
    ...
    def test_synthesis_failure(self, make_client, fake_db):
        """Provider 可用但合成失败（返回 None）→ TTS_UNAVAILABLE。"""
        client = _client(make_client, provider=FakeTtsProvider.failing())
        resp = client.post("/audio/tts", json={"text": DEFAULT_TEXT})
        body = resp.json()
        assert body["success"] is False
        assert body["code"] == "TTS_UNAVAILABLE"
    ...
    def test_response_missing_audio(self, make_client, fake_db):
        """Provider 返回原始响应但缺 Audio 字段 → TTS_UNAVAILABLE。"""
        client = _client(make_client, provider=FakeTtsProvider(result={"RequestId": "req-x"}))
        resp = client.post("/audio/tts", json={"text": DEFAULT_TEXT})
        body = resp.json()
        assert body["success"] is False
        assert body["code"] == "TTS_UNAVAILABLE"
...
class TestTtsSemanticSearchReserved:
    ...
    def test_reserved_returns_empty_hits(self, make_client, fake_db):
    ...
    def test_invalid_text(self, make_client, fake_db):
    ...
    def test_top_k_out_of_range(self, make_client, fake_db):
    ...
    def test_tts_miss_invokes_semantic_lookup(self, make_client, fake_db, monkeypatch):
