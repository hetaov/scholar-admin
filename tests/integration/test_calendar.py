...
class TestStudyCalendar:
    ...
    def test_heatmap_aggregation(self, make_client, fake_db):
    ...
    def test_streak_consecutive(self, make_client, fake_db):
    ...
    def test_streak_zero_when_today_missing(self, make_client, fake_db):
    ...
    def test_streak_stops_at_gap(self, make_client, fake_db):
        """昨天无记录 → 只算今天(中断即停)。"""
        fake_db.add("study_attempt", _attempt("a1", _ts(0)))
        fake_db.add("study_attempt", _attempt("a2", _ts(2)))
        client = make_client(tracking_router)
    ...
    def test_window_days_limit(self, make_client, fake_db):
    ...
    def test_default_days_30(self, make_client, fake_db):
    ...
    def test_empty_no_attempts(self, make_client, fake_db):
    ...
    def test_days_invalid_400(self, make_client, fake_db):
        client = make_client(tracking_router)
        for bad in ("0", "91", "-1"):
